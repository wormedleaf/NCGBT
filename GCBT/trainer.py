...
class GCBTTrainer(Trainer):

    def __init__(self, config, model):
        super(GCBTTrainer, self).__init__(config, model)
    ...
    def topK_accuracy(prediction, y_test, classes, rank_k=10):
        ...
        sortedIndices = []
        pred_classes = []
        for bugLabels in prediction:
            sortedIndices.append(
                sorted(range(len(bugLabels)), key=lambda x: bugLabels[x], reverse=True)
            )
        for k in range(1, rank_k + 1):
            id = 0
            trueNum = 0
            for sortedIndex in sortedIndices:
                pred_classes.append(classes[sortedIndex[:k]])
                if np.argmax(y_test[id]) in sortedIndex[:k]:
                    trueNum += 1
                id += 1
